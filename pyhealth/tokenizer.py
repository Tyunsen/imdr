...
class Tokenizer:
    ...
    def batch_encode_3d(
        ...
        # truncation: Tuple[bool, bool] = (False, False),
        max_length: Tuple[int, int] = (10, 512),
        pad_visit_length: bool = False,
    ):
    ...
    def batch_encode_4d(
        self,
        batch: List[List[List[List[str]]]],
        padding: Tuple[bool, bool, bool] = (True, True, True),
        truncation: Tuple[bool, bool, bool] = (True, True, True),
        max_length: Tuple[int, int, int] = (10, 512, 128),
    ):
        """Converts a list of lists of lists of lists of tokens (4D) to indices.
        Args:
            ...
            max_length: a tuple of three integers indicating the maximum length of the
                ...
                if truncation is False.
        """
        # Apply truncation on first dimension
        if truncation[0]:
            batch = [tokens[-max_length[0]:] for tokens in batch]
        
        # Apply truncation on second dimension
        if truncation[1]:
            batch = [
                [tokens[-max_length[1]:] for tokens in visits] 
                for visits in batch
            ]
        
        # Apply truncation on third dimension
        if truncation[2]:
            batch = [
                [
                    [tokens[-max_length[2]:] for tokens in subvisits]
                    for subvisits in visits
                ]
                for visits in batch
            ]

        # Apply padding on first dimension
        if padding[0]:
            batch_max_length = max([len(tokens) for tokens in batch])
            batch = [
                tokens + [[["<pad>"]]] * (batch_max_length - len(tokens))
        ...
        # Apply padding on second dimension
        if padding[1]:
            batch_max_length = max(
                [max([len(tokens) for tokens in visits]) for visits in batch]
            )
        ...
        # Apply padding on third dimension
        if padding[2]:
            batch_max_length = max(
                [max([max([len(tokens) for tokens in subvisits]) for subvisits in visits]) for visits in batch]
            )
            batch = [
                [
                    [
                        tokens + ["<pad>"] * (batch_max_length - len(tokens))
                        for tokens in subvisits
                    ]
                    for subvisits in visits
                ]
                for visits in batch
