import sys
sys.path.append('/home/tys/sddr2')
from pyhealth.metrics.drug_recommendation import ddi_rate_score_per_sample
...
def multilabel_metrics_fn(
    ...
    for metric in metrics:
        ...
        elif metric == "jaccard_samples_batch":
            intersection = np.logical_and(y_true, y_pred)
            union = np.logical_or(y_true, y_pred)
            jaccard = np.sum(intersection, axis=1) / np.sum(union, axis=1)
            output["jaccard_samples_batch"] = jaccard
        ...
        elif metric == "ddi_score_batch":
            ddi_adj = np.load(os.path.join(CACHE_PATH, 'ddi_adj.npy'))
            y_pred_temp = [np.where(item)[0] for item in y_pred]
            output["ddi_score_batch"] = np.array(ddi_rate_score_per_sample(y_pred_temp, ddi_adj))
...
if __name__ == "__main__":
    ...
    y_true = np.random.randint(2, size=(1000, 193))
    y_prob = np.random.random(size=(1000, 193))
    # print(multilabel_metrics_fn(y_true, y_prob, metrics=all_metrics))
    t = multilabel_metrics_fn(y_true, y_prob, metrics=["ddi_score","ddi_score_batch"])
    t1 = t["ddi_score"]
    t2 = t["ddi_score_batch"]
    t2_mean = np.mean(t2)
    print(1)
