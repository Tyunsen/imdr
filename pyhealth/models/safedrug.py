...
class SafeDrugLayer(nn.Module):
    ...
    def calculate_loss(
            self, logits: torch.Tensor, y_prob: torch.Tensor, labels: torch.Tensor, label_index: torch.Tensor,
    ) -> torch.Tensor:
        ...
        if cur_ddi_rate > self.target_ddi:
            ...
            loss = beta * (self.multiloss_weight * loss_multi + (1 - self.multiloss_weight) * bce_loss) + (
                    1 - beta) * add_loss
        else:
    ...
    def forward(
        ...
        loss = self.calculate_loss(logits, y_prob, drugs, drug_indexes)
...
class SafeDrug(BaseModel):
    ...
    def forward(
        ...
        loss, y_prob = self.safedrug(patient_emb, labels, mask, index_labels)
