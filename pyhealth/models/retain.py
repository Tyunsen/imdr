...
class RETAINLayer(nn.Module):
    ...
    def __init__(
        ...
        self.alpha_gru = nn.GRU(feature_size, feature_size, batch_first=True, num_layers=2, dropout=dropout)
        self.beta_gru = nn.GRU(feature_size, feature_size, batch_first=True, num_layers=2, dropout=dropout)
...
class RETAIN(BaseModel):
    ...
    def __init__(
        ...
        output_size = self.get_output_size(self.label_tokenizer)
        num_layers = 2  # 设置全连接层的层数

        layers = []
        input_size = len(self.feature_keys) * self.embedding_dim

        for i in range(num_layers):
            layers.append(nn.Linear(input_size, self.embedding_dim))
            layers.append(nn.ReLU())
            input_size = self.embedding_dim

        layers[-1] = nn.Linear(self.embedding_dim, output_size)
    ...
    def forward(self, **kwargs) -> Dict[str, torch.Tensor]:
        ...
        for feature_key in self.feature_keys:
            ...
            # for case 2: [[code1, code2], [code3, ...], ...]
                ...
                # t = kwargs[feature_key]
                # for patient in kwargs[feature_key]:
                #     t = len(patient)
                #     if len(patient) == 0 or patient[0] == []:
                #         print(1)

                # x = self.feat_tokenizers[feature_key].batch_encode_3d(kwargs[feature_key],truncation=(True, True))
                ...
                # # 判断 zero_indices 是否有值
                # if zero_indices.numel() > 0:
                #     print("zero_indices 有值:", zero_indices)
                # else:
                #     print("zero_indices 没有值")
