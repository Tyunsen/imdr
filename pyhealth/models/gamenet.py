...
class GAMENetLayer(nn.Module):
    ...
    def __init__(
            ...
            dropout: float = 0.5,
            multiloss_weight: float = 0.01,
            kp: float = 0.5,
            target_ddi: float = 0.1,
            **kwargs,
    ):
    ...
    def forward(
        ...
        a_c = torch.softmax(torch.mm(query, MB.t()), dim=-1) # (batch,hidden)*(hidden,label_num)=(batch,label_num)
        o_b = torch.mm(a_c, MB) # (batch,label_num)*(label_num,hidden)=(batch,hidden)
        ...
        bce_loss = self.bce_loss_fn(logits, curr_drugs)
        ...
        if cur_ddi_rate > self.target_ddi:
            rnd = np.exp((self.target_ddi - cur_ddi_rate) / self.kp)
            if np.random.rand(1) < rnd:
                loss = self.get_ddi_loss(y_prob)
            else:
                loss = self.multiloss_weight * loss_multi + (1 - self.multiloss_weight) * bce_loss
        else:
            loss = self.multiloss_weight * loss_multi + (1 - self.multiloss_weight) * bce_loss
...
class GAMENet(BaseModel):
    ...
    def forward(
        ...
        # drugs_hist = self.label_tokenizer.batch_encode_3d(
        #     drugs_hist, padding=(False, False), truncation=(False, False)
        # )
        ...
        # process drugs
        loss, y_prob = self.gamenet(queries, prev_drugs, curr_drugs, index_labels,mask)
